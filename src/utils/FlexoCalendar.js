...
2016-02-5
*/
const jQuery = require('./jquery.min')
;+(function ($) {
  'use strict'
  ...
  Calendar.prototype.init = function (element, options) {
    ...
    $(element).on('click', '.prev, .next', options, function () {
      ...
      if (
        $.isArray(allowDate) &&
        ((year == allowDate[0].split('-')[0] &&
          month == allowDate[0].split('-')[1]) ||
          (year == allowDate[1].split('-')[0] &&
            month == allowDate[1].split('-')[1]))
      )
        return
      ...
      $(options.target)
        .find('table.flexoCalendar tbody')
        .empty()
        .append(calendar)
        .trigger('el.create')
      ...
      if (this.className == 'prev' && options.ongoprev) {
        options.ongoprev.call(
          options.target,
          $(this).next('.current-year').data('year'),
          $(this).data('month')
        )
      }
      if (this.className == 'next' && options.ongonext) {
        options.ongonext.call(
          options.target,
          $(this).prev('.current-year').data('year'),
          $(this).data('month')
        )
      }
    })
    ...
    if (options.select) {
      $(element).on('click', 'table>tbody>tr>td', options, function () {
        ...
        if (options.disOtherMonth) {
          if (this.className.split(' ')[0] != 'other-month') {
  ...
  Calendar.prototype.bulidCH = function (options) {
    ...
    if (this.type == 'normal') {
      calendar +=
        '<thead><tr class="calendar-hd"><th class="prev" data-month="' +
        prevdate +
        '"><i class="' +
        prev +
        '"></i></th><th class="current-year" data-year="' +
        year +
        '-' +
        this.dispose(month) +
        '" colspan="5">' +
  ...
  Calendar.prototype.changeCH = function (element, options) {
    ...
    if (type == 'weekly') {
      ...
      $(options.target)
        .find('.prev')
        .data('month', prevdate)
        .attr('data-month', prevdate)
        .data('week', prevWeek)
        .attr('data-week', prevWeek)
      $(options.target)
        .find('.next')
        .data('month', nextdate)
        .attr('data-month', nextdate)
        .data('week', nextWeek)
        .attr('data-week', nextWeek)
    } else {
      $(options.target)
        .find('.prev')
        .data('month', prevdate)
  ...
  Calendar.prototype.bulidFull = function (options, year, month) {
    ...
    var prevMonthLen = eli.getMonthLen(
      prevdate.split('-')[0],
      prevdate.split('-')[1]
    )
    ...
    for (var j = 0; j < 42; j++) {
      ...
      if (j >= firDay && j < firDay + monthLen) {
        if (options.today) {
          if (
            j == parseInt(options.crtdate) + firDay - 1 &&
            options.realyear == year &&
            parseInt(options.realmonth) == month
          ) {
            if (
              selectDate &&
              selectDate[0] == year &&
              selectDate[1] == month &&
              selectDate[2] == parseInt(options.crtdate)
            ) {
              calendar +=
                '<td class="current-month current-day selected-day tdday day' +
                (j % 7) +
                '" data-time="' +
                year +
                '-' +
                this.dispose(month) +
                '-' +
                this.dispose(j - firDay + 1) +
                '"><span class="day">' +
                (j - firDay + 1) +
                '</span></td>'
            } else {
              calendar +=
                '<td class="current-month current-day tdday day' +
                (j % 7) +
                '" data-time="' +
                year +
                '-' +
            ...
            if (
              ...
              selectDate[0] == year &&
              selectDate[1] == month &&
              selectDate[2] == j - firDay + 1
            ) {
              calendar +=
                '<td class="current-month selected-day tdday day' +
                (j % 7) +
                '" data-time="' +
                year +
                '-' +
              ...
              calendar +=
                '<td class="current-month tdday day' +
                (j % 7) +
                '" data-time="' +
                year +
                '-' +
      ...
      //上个月
        ...
        if (
          selectDate &&
          selectDate[0] + '-' + selectDate[1] == prevdate &&
          selectDate[2] == prevMonthLen - (firDay - j - 1)
        ) {
          calendar +=
            '<td class="other-month selected-day prev-month day' +
            (j % 7) +
            '" data-time="' +
            prevdate +
            '-' +
            this.dispose(prevMonthLen - (firDay - j - 1)) +
            '"><span class="day">' +
            (prevMonthLen - (firDay - j - 1)) +
            '</span></td>'
        } else {
          calendar +=
            '<td class="other-month prev-month day' +
            (j % 7) +
            '" data-time="' +
            prevdate +
            '-' +
      ...
      //下个月
        ...
        if (
          selectDate &&
          selectDate[0] + '-' + selectDate[1] == nextdate &&
          selectDate[2] == j - monthLen - firDay + 1
        ) {
          calendar +=
            '<td class="other-month selected-day next-month day' +
            (j % 7) +
            '" data-time="' +
            nextdate +
            '-' +
            this.dispose(j - monthLen - firDay + 1) +
            '"><span class="day">' +
            (j - monthLen - firDay + 1) +
            '</span></td>'
        } else {
          calendar +=
            '<td class="other-month next-month day' +
            (j % 7) +
            '" data-time="' +
            nextdate +
            '-' +
  ...
  Calendar.prototype.bulidWeekly = function (options, year, month) {
    ...
    var endDay =
      parseInt(startDay) + 6 > parseInt(prevMonthLen)
        ? parseInt(startDay) + 6 - parseInt(prevMonthLen)
        : parseInt(startDay) + 6
    ...
    if (selectDate) {
      ...
      selectDate[2] =
        selectDate[2] == 'each'
          ? 'week1'
          : selectDate[2].indexOf('week') != -1
          ? selectDate[2]
          : parseInt(selectDate[2])
      if (typeof selectDate[2] == 'number') {
        selectDate[2] = eli.getMonthWeek(
          selectDate[0],
          selectDate[1],
          selectDate[2]
        )
    ...
    for (var j = 0; j < 5; j++) {
      if (j != 0 && parseInt(startDay) >= parseInt(endDay)) {
        if (selectDate) {
          crt = 'week' + (j + 1)
          if (
            ($.isArray(selectDate[2]) &&
              year == selectDate[2][0][0] &&
              month == selectDate[2][0][1] &&
              selectDate[2][0][2] == crt) ||
            (year == selectDate[0] &&
              month == selectDate[1] &&
              selectDate[2] == crt)
          ) {
            calendar +=
              '<tr><td class="selected-week tweek week' +
              (j + 1) +
              '" data-time="' +
              startYear +
              '-' +
              this.dispose(startMonth) +
              '-' +
              this.dispose(startDay) +
              ',' +
              endYear +
              '-' +
              this.dispose(month) +
              '-' +
              this.dispose(endDay) +
              '" colspan="7"><div><span class="week">第' +
              numbers[j] +
              '周</span><span class="process">' +
              this.dispose(startMonth) +
              '/' +
              this.dispose(startDay) +
              '~' +
              this.dispose(nextMonth) +
              '/' +
              this.dispose(endDay) +
              '</span></div></td></tr>'
          } else {
            calendar +=
              '<tr><td class="tweek week' +
              (j + 1) +
              '" data-time="' +
              startYear +
              '-' +
        ...
        options.today &&
        ((parseInt(options.realmonth) == startMonth &&
          startDay <= parseInt(options.crtdate)) ||
          (parseInt(options.realmonth) != startMonth &&
            startDay >= parseInt(options.crtdate))) &&
        parseInt(options.crtdate) <= endDay &&
        options.realyear == year &&
        options.realmonth == month
      ) {
        if (selectDate) {
          crt = 'week' + (j + 1)
          if (
            ($.isArray(selectDate[2]) &&
              ((year == selectDate[2][0][0] &&
                ...
                selectDate[2][0][2] == crt) ||
                (selectDate[2][1] &&
                  year == selectDate[2][1][0] &&
                  month == selectDate[2][1][1] &&
                  selectDate[2][1][2] == crt))) ||
            (year == selectDate[0] &&
              month == selectDate[1] &&
              selectDate[2] == crt)
          ) {
            calendar +=
              '<tr><td class="current-week current-day selected-week tweek week' +
              (j + 1) +
              '" data-time="' +
              startYear +
              '-' +
            ...
            calendar +=
              '<tr><td class="current-week current-day tweek week' +
              (j + 1) +
              '" data-time="' +
              startYear +
              '-' +
      ...
      if (j == 0 && firDay != 1) {
        startMonth =
          parseInt(startMonth) + 1 > 12 ? '1' : parseInt(startMonth) + 1
  ...
  Calendar.prototype.bulidMonthly = function (options, year, month) {
    ...
    selectDate =
      typeof selectDate == 'string'
        ? selectDate.indexOf('-') != -1 && selectDate.indexOf('week') == -1
          ? selectDate.split('-')
          : null
        : null
    if (selectDate) {
    ...
    for (var j = 0; j < 12; j++) {
      ...
      if (
        options.today &&
        j + 1 == options.realmonth &&
        options.realyear == year
      ) {
        if (selectDate && selectDate[0] == year && selectDate[1] == j + 1) {
          calendar +=
            '<td colspan=2 class="current-month current-day selected-month selected-day tmonth month' +
            this.dispose(j + 1) +
            '" data-time="' +
            year +
            '-' +
          ...
          calendar +=
            '<td colspan=2 class="current-month current-day tmonth month' +
            this.dispose(j + 1) +
            '" data-time="' +
            year +
            '-' +
        ...
        calendar +=
          '<td colspan=2 class="selected-month selected-day tmonth month' +
          this.dispose(j + 1) +
          '" data-time="' +
          year +
          '-' +
        ...
        calendar +=
          '<td colspan=2 class="tmonth month' +
          this.dispose(j + 1) +
          '" data-time="' +
          year +
          '-' +
  ...
  Calendar.prototype.getNext = function (year, month, type) {
    ...
    if (type == 'monthly') {
      ...
      nextdate =
        parseInt(month) == 12
          ? parseInt(year) + 1 + '-01'
          : year + '-' + this.dispose(parseInt(month) + 1)
    }
    return nextdate
  }
  ...
  var Plugin = function (option) {
    return this.each(function () {
      ...
      if (!data)
        $this.data('ellie.calendar', (data = new Calendar(this, option)))
    })
  }
